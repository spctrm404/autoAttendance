...
int[][] seqB = {
  ...
  {1046, 163, 139,211,243}, // 전체확인 버튼
  {1490, 244, 252, 65, 92}, // 완료 버튼
  {1046, 183, 139,211,243}, // 완료확인 버튼
  {1118, 163, 139,211,243}, // 출석확인 버튼
  {1730, 244, 46, 49, 58} // 목록 버튼
};

boolean colorChk (int _x, int _y, int _critR, int _critG, int _critB) {
  Color c = r.getPixelColor(_x + offset[0], _y + offset[1]);
  if (c.getRed() == _critR 
    && c.getGreen() == _critG 
    && c.getBlue() == _critB) {
    return true;
  ...
  while (cnt < times) {
    ...
    if (stpA == 0) {
      ...
      if (millis() - lastMillis > 1000) {
        if (colorChk(dataARoot[0], dataARoot[1], 
          dataARoot[2], dataARoot[3], dataARoot[4])) { 
          r.mouseMove(dataA[0]+offset[0], dataA[1]+offset[1]);
      ...
      if (millis() - lastMillis > 1000) {
        if (colorChk(dataARoot[0], dataARoot[1], 
          ...
          if (colorChk(dataA[0], dataA[1] + dataA[5] * nthClass, 
            ...
            r.mouseMove(dataA[0]+offset[0], dataA[1] + dataA[5] * nthClass+offset[1]);
            r.mousePress(InputEvent.BUTTON1_MASK);
            r.mouseRelease(InputEvent.BUTTON1_MASK);
            println("enter");
            loopInLoop(millis());
  ...
  while (stpB < seqB.length) {
    ...
    if (stpB == 0) {
      ...
      if (millis() - lastMillis > 1000) {
        if (colorChk(dataBRoot[0], dataBRoot[1], 
          dataBRoot[2], dataBRoot[3], dataBRoot[4])) {
          r.mouseMove(dataB[0]+offset[0], dataB[1]+offset[1]);
          r.mousePress(InputEvent.BUTTON1_MASK);
          r.mouseRelease(InputEvent.BUTTON1_MASK);
          lastMillis = millis();
          stpB++;
